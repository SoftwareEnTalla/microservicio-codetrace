...
export class CodetraceCommandRepository implements IEventHandler<BaseEvent>{
  ...
  async create(entity: Codetrace): Promise<Codetrace> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvt1 = new CodetraceCreatedEvent(result.id, {
        instance: result,
        metadata: {
  ...
  async bulkCreate(entities: Codetrace[]): Promise<Codetrace[]> {
    ...
    if (this.shouldPublishEvent()) {
      const __dualEvts2 = result.map((el)=>new CodetraceCreatedEvent(el.id, {
        instance: el,
        metadata: {
  ...
  async update(
    ...
    if(instance && this.shouldPublishEvent()) {
      ...
      const __dualEvt3 = new CodetraceUpdatedEvent(instance.id, {
          instance: instance,
          metadata: {
    ...
    for (const entity of entities) {
      if (entity.id) {
        ...
        if (updatedEntity) {
          ...
          if (this.shouldPublishEvent()) {
            const __dualEvt4 = new CodetraceUpdatedEvent(updatedEntity.id, {
  ...
  async delete(id: string): Promise<DeleteResult> {
     ...
     if (this.shouldPublishEvent()) {
       ...
       const __dualEvt5 = new CodetraceDeletedEvent(id, {
        instance: entity,
        metadata: {
